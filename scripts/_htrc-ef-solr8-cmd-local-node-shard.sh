...
solr_server_dir="$SOLR8_SERVER_BASE_JETTY_DIR/solr-server-$solr_host-$solr_port"
...
if [ $solr_cmd = "status" ] ; then    
    echo "Running Solr8 command '$solr_cmd' for cloud node: $solr_host"
else
...
if [ "x$SOLR8_AUTH_TYPE" != "x" ] ; then    
    ...
    if [ -f "$SOLR8_TOP_LEVEL_HOME/server/etc/realm.properties" ] ; then
	echo "  Setting environment variables SOLR_AUTH_TYPE and SOLR_AUTHENTICATION_OPTS"
	...
	echo "Environment variable SOLR8_AUTH_TYPE not set"
	echo "AND failed to find: $SOLR8_TOP_LEVEL_HOME/server/etc/realm.properties"
...
if [ $solr_cmd = "start" ] || [ $solr_cmd = "restart" ] ; then
    ...
    "$SOLR8_TOP_LEVEL_HOME/bin/solr" $solr_cmd -cloud -z $zookeeper_server_list \
      -h $solr_host -p $solr_port -d "$solr_server_dir" -s "$solr_home_shard_dir"
    ...
    if [ "$numSolrs" == "0" ]; then
	echo "****" >&2
	echo "* No solr-*.pid files in $SOLR_PID_DIR => No running Solr8 nodes detected on this host" >&2
	echo "****" >&2
	echo 1
    else
